      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-20">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 text-xs font-bold uppercase tracking-wider mb-6">
            <Activity className="w-3 h-3" /> Core Engine #1: Risk Attribution
          </div>
          <h3 className="text-4xl font-extrabold text-gray-900 tracking-tight sm:text-6xl font-display">
            Patient <span className="text-indigo-600">Risk Assessment</span>.
          </h3>
          <p className="mt-6 text-xl text-gray-500 max-w-2xl mx-auto leading-relaxed">
            Multi-modal risk indexing across HLA genotype, fecal specimen abundance, and baseline inflammatory markers. This protocol generates the "Steady State" risk from which the Simulator begins.
          </p>
        </div>
                  ...
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Model Confidence:</div>
                        <div className={cn(
                          "px-2 py-0.5 rounded text-[8px] font-bold uppercase",
                          result.confidenceLevel === 'High' ? "bg-emerald-500/20 text-emerald-400" :
                          result.confidenceLevel === 'Medium' ? "bg-amber-500/20 text-amber-400" : "bg-red-500/20 text-red-400"
                        )}>
                          {result.confidenceLevel} ({result.confidenceInterval})
                        </div>
                      </div>
                    </div>
